=head1 NAME
   ...
   GeneDataStructure::CreateKnownGenes.pm
...
use KnownGenesCodingTranscript;
...
=item new
   ...
   Constructor to build the array of KnownGenesCodingTranscript.pm
   ...
   _arrayOfTranscripts         : all of the KnownGenesCodingTranscript.pm
   _arrayOfTranscriptsPerChr   : The KnownGenesCodingTranscript.pm on a given chromosome
...
sub new{
  ...
  if(!(-e ChromosomeIndexing::getBaseDir().$self->{_species}."/database/knownGene.txt")){
    die "The KnownGene file ".ChromosomeIndexing::getBaseDir().$self->{_species}."/database/knownGene.txt does not exists\n";
  }

  $self->{_arrayOfTranscripts}=[];
  $self->{_arrayOfTranscriptsPerChr}=[];
  my $file=ChromosomeIndexing::getBaseDir().$self->{_species}."/database/knownGene.txt";

  open(INFO, $file) or die ("Can't open $file ");
  while (my $line = <INFO>) {
    my $trans;
    if($self->{_baseDir}){
      $trans=KnownGenesCodingTranscript->new(knowngeneline   => $line,
...
#
#   Subroutine to return an array of KnownGenesCodingTranscript.pm
...
#    die "CreateKnownGene.pm : The index $index does not exists\n";
#  }
#  return internalGetArrayOfTranscriptsForGivenChrIndex($index);
#}
#
